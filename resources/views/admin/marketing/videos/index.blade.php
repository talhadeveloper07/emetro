            ...
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                @forelse($videos as $video)
                    <div class="col position-relative">
                        <a href="{{ $video->document ? asset('storage/'.$video->document) : ($video->video ? asset('storage/'.$video->video) : '#') }}" 
                           target="_blank" 
                           class="text-decoration-none text-dark w-100">

                            <div class="marketing border rounded shadow-sm p-2 hover-shadow">
                                {{-- Thumbnail --}}
                                <div class="w-30" style="width:100%;">
                                    @if($video->image)
                                        <img src="{{ asset('storage/'.$video->image) }}" alt="thumbnail" class="rounded" style="max-height:60px;object-fit:contain;">
                                    @elseif($video->image)
                                ...
                                {{-- Details --}}
                                <div class="ms-2 w-70" style="width:100%;">
                                    <h5 class="mb-1 text-start">{{ $video->title }}</h5>
                                    <p class="text-muted mb-0" style="font-size:12px;">{!! $video->description !!}</p>
                                </div>
                            </div>
                        </a>
                        ...
                        <button class="btn btn-link p-0 m-0"
                            ...
                            data-type="{{ $video->content_type }}"
                            data-image="{{ $video->image ? asset('storage/'.$video->image) : '' }}"
                            data-doc="{{ $video->document ? asset('storage/'.$video->document) : '' }}"
    ...
    <div class="modal-content">
      <form id="mediaForm" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-header">
          <h5 class="modal-title" id="modalTitleText">Add New Video</h5>
...
function openCreateModal() {
    ...
    document.getElementById('modalTitleText').innerText = 'Add New Video';
...
function openEditvideoModal(el) {
    ...
    // Fill modal
    document.getElementById('modalTitleText').innerText = 'Edit Video';
