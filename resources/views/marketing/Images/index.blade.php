            ...
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                @forelse($images as $image)
                    <div class="col position-relative">
                        <a href="{{ $image->document ? asset('storage/'.$image->document) : ($image->image ? asset('storage/'.$image->image) : '#') }}" 
                           target="_blank" 
                           class="text-decoration-none text-dark w-100">
                            ...
                            <div class="marketing border rounded shadow-sm p-2 d-flex align-items-center hover-shadow">
                                ...
                                <div class="w-30" style="width:30%;">
                                    @if($image->image)
                                        <img src="{{ asset('storage/'.$image->image) }}" alt="thumbnail" class="rounded" style="max-height:60px;object-fit:contain;">
                                ...
                                <div class="ms-2 w-70" style="width:70%;">
                                    <h5 class="mb-1 text-start">{{ $image->title }}</h5>
                                    <p class="text-muted mb-0" style="font-size:12px;">{{ $image->description }}</p>
                                </div>
                            </div>
                        </a>
                        ...
                        <button class="btn btn-link p-0 m-0" 
                            onclick="openEditImageModal(
                                {{ $image->id }},
                                '{{ addslashes($image->title) }}',
                                '{{ addslashes($image->description) }}',
                                '{{ $image->content_type }}',
                                '{{ $image->image ? asset('storage/'.$image->image) : '' }}',
                                '{{ $image->document ? asset('storage/'.$image->document) : '' }}'
                            )">
    ...
    <div class="modal-content">
      <form id="mediaForm" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-header">
          <h5 class="modal-title" id="modalTitleText">Add New Image</h5>
...
function openCreateModal() {
    ...
    document.getElementById('modalTitleText').innerText = 'Add New Image';
    ...
    document.getElementById('mediaContentType').value = 'image';
    document.getElementById('currentImg').style.display = 'none';
    document.getElementById('currentDoc').innerHTML = '';
    new bootstrap.Modal(modal).show();
}

function openEditImageModal(id, title, description, contentType, imageUrl, docUrl) {
    const modal = document.getElementById('mediaModal');
    document.getElementById('modalTitleText').innerText = 'Edit Media';
