            ...
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                @forelse($logos as $logo)
                    <div class="col position-relative">
                        <a href="{{ $logo->document ? asset('storage/'.$logo->document) : ($logo->logo ? asset('storage/'.$logo->logo) : '#') }}" 
                           target="_blank" 
                           class="text-decoration-none text-dark w-100">
                            ...
                            <div class="marketing border rounded shadow-sm p-2 d-flex align-items-center hover-shadow">
                                ...
                                <div class="w-30" style="width:30%;">
                                    @if($logo->image)
                                        <img src="{{ asset('storage/'.$logo->image) }}" alt="thumbnail" class="rounded" style="max-height:60px;object-fit:contain;">
                                    @elseif($logo->image)
                                ...
                                <div class="ms-2 w-70" style="width:70%;">
                                    <h5 class="mb-1 text-start">{{ $logo->title }}</h5>
                                    <p class="text-muted mb-0" style="font-size:12px;">{{ $logo->description }}</p>
                                </div>
                            </div>
                        </a>
                        ...
                        <button class="btn btn-link p-0 m-0" 
                            onclick="openEditlogoModal(
                                {{ $logo->id }},
                                '{{ addslashes($logo->title) }}',
                                '{{ addslashes($logo->description) }}',
                                '{{ $logo->content_type }}',
                                '{{ $logo->image ? asset('storage/'.$logo->image) : '' }}',
                                '{{ $logo->document ? asset('storage/'.$logo->document) : '' }}'
                            )">
    ...
    <div class="modal-content">
      <form id="mediaForm" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-header">
          <h5 class="modal-title" id="modalTitleText">Add New Logo</h5>
...
function openCreateModal() {
    ...
    document.getElementById('modalTitleText').innerText = 'Add New Logo';
    ...
    document.getElementById('mediaContentType').value = 'logo';
    document.getElementById('currentImg').style.display = 'none';
    document.getElementById('currentDoc').innerHTML = '';
    new bootstrap.Modal(modal).show();
}

function openEditlogoModal(id, title, description, contentType, logoUrl, docUrl) {
    const modal = document.getElementById('mediaModal');
    document.getElementById('modalTitleText').innerText = 'Edit Logo';
