            ...
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                @forelse($presentations as $presentation)
                    <div class="col position-relative">
                        <a href="{{ $presentation->document ? asset('storage/'.$presentation->document) : ($presentation->presentation ? asset('storage/'.$presentation->presentation) : '#') }}" 
                           target="_blank" 
                            ...
                            <div class="marketing border rounded shadow-sm p-2 d-flex align-items-center hover-shadow">
                                ...
                                <div class="w-30" style="width:30%;">
                                    @if($presentation->image)
                                        <img src="{{ asset('storage/'.$presentation->image) }}" alt="thumbnail" class="rounded" style="max-height:60px;object-fit:contain;">
                                ...
                                <div class="ms-2 w-70" style="width:70%;">
                                    <h5 class="mb-1 text-start">{{ $presentation->title }}</h5  >
                                    <p class="text-muted mb-0" style="font-size:12px;">{{ $presentation->description }}</p>
                                </div>
                            </div>
                        </a>
                        ...
                        <button class="btn btn-link p-0 m-0" 
                            onclick="openEditpresentationModal(
                                {{ $presentation->id }},
                                '{{ addslashes($presentation->title) }}',
                                '{{ addslashes($presentation->description) }}',
                                '{{ $presentation->content_type }}',
                                '{{ $presentation->image ? asset('storage/'.$presentation->image) : '' }}',
                                '{{ $presentation->document ? asset('storage/'.$presentation->document) : '' }}'
                            )">
                        ...
                        <button class="btn btn-link p-0 m-0 delete" 
                            onclick="openDeleteModal({{ $presentation->id }})">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                 viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                 stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
    ...
    <div class="modal-content">
      <form id="mediaForm" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-header">
          <h5 class="modal-title" id="modalTitleText">Add Product Presentation</h5>
...
function openCreateModal() {
    ...
    document.getElementById('modalTitleText').innerText = 'Add Product Presentation';
    ...
    document.getElementById('mediaContentType').value = 'presentation';
    document.getElementById('currentImg').style.display = 'none';
    document.getElementById('currentDoc').innerHTML = '';
    new bootstrap.Modal(modal).show();
}

function openEditpresentationModal(id, title, description, contentType, presentationUrl, docUrl) {
    const modal = document.getElementById('mediaModal');
    document.getElementById('modalTitleText').innerText = 'Edit Product Presentation';
    ...
    img.style.display = presentationUrl ? 'block' : 'none';
